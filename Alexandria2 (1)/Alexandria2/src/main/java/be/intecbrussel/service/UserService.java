...
public class UserService {
    ...
    public Optional<User> searchForMemberbyIDnumber(Integer searchID) {

        UserRepository myUserRepository = new UserRepository();
        List <User> userList = myUserRepository.getUserList();


        Optional<List> allUsersWithSameIDOptional = Optional.of(userList.stream()
                .filter(u -> u.getUserId() == searchID)
                .toList()); //getting users with same ID
        ...
        return Optional.of(allUsersWithSameID).map(u -> u.get(0));
