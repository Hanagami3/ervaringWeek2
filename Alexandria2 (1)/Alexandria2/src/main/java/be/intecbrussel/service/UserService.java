...
import be.intecbrussel.utils.SearchTools;
...
public class UserService {
    ...
    public void UpdateMemberInfo() {   //Jonathan
        ...
        List<User> userList = myUserRepository.getUserList();
        ...
        myUserRepository.addUser(userNewWithOldID);

        myStringScanner.close();
        myIntScanner.close();
    }

    public Optional<List> searchForMemberbyIDnumber(Integer searchID) {

        UserRepository myUserRepository = new UserRepository();
        List <User> userList = myUserRepository.getUserList();


        Optional<List> allUsersWithSameIDOptional = Optional.of(userList.stream().filter(u -> u.getUserId() == searchID).toList()); //getting users with same ID
        ...
        return Optional.of(allUsersWithSameID);
