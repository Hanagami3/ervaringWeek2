...
public class BookService {
    ...
    public boolean reservedBook(int bookIDNO, int userId){
        ...
        if (optionalBook.isPresent() && optionalUser.isPresent()) {
            ...
            if (book.getStatus() == BookStatus.AVAILABLE) {
                ...
                book.setReservedBy(Integer.parseInt(String.valueOf(optionalUser)));
    ...
    public void returnBookByBookIDNO(int bookIDNO) {
        Scanner scanner = new Scanner(System.in);
        ...
        if (bookToReturn.isPresent() && optionalUser.isPresent()) {
            ...
            if (book.getStatus() == BookStatus.BORROWED) {
                int borrowedByUserId = book.getBorrowedBy();
                if (borrowedByUserId == userId) {
                    ...
                    book.setBorrowedBy(0); // Set borrowedBy to 0 to indicate no user
    ...
    public void returnAllBooks() {
        ...
        for (Book book : borrowedBooks) {
            double fineAmount = calculateFine(LocalDate.now(), book.getDueDate());
            if (fineAmount > 0) {
                User user = userService.searchForMemberbyIDnumber(book.getBorrowedBy()).orElse(null);
                if (user != null) {
...
//    private void addFine(double amount) {
//    }
}
